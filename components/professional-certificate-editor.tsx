...
export function ProfessionalCertificateEditor({ onStateChange, initialTemplate, templateId }: ProfessionalEditorProps) {
  ...
  // Debounced Auto-Save to localStorage and parent state update
  useEffect(() => {
    if (!isInitialized) return
    ...
    const handler = setTimeout(() => {
      ...
      try {
        ...
        onStateChange(currentState, true) // Notify parent component that there are unsaved changes
      } catch (error) {
        console.error("Failed to save state to localStorage", error)
        toast({
          title: "Erro de Salvamento",
          description: "Não foi possível salvar as alterações localmente. O localStorage pode estar cheio.",
          variant: "destructive",
        })
      }
    }, 500)
