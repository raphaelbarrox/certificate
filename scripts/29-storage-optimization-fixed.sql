...
-- Removido índice com predicado problemático e criado índice simples
-- Índice simples para otimizar consultas de performance sem predicado
CREATE INDEX IF NOT EXISTS idx_issued_certificates_performance_simple 
ON issued_certificates (issued_at DESC, pdf_url);

-- Índice adicional para consultas recentes
CREATE INDEX IF NOT EXISTS idx_issued_certificates_recent 
ON issued_certificates (issued_at) 
WHERE issued_at >= '2024-01-01'::DATE;
