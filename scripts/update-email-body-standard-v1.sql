...
-- Atualizar TODOS os templates com o novo corpo de email HTML padrão
  ...
  jsonb_set(
    jsonb_set(
      jsonb_set(
        ...
        '"Marina L. - Therapist University"'::jsonb
      ),
      '{emailConfig,senderEmail}', 
      '"certificados@therapist.international"'::jsonb
    ),
    '{emailConfig,subject}', 
    '"🎉 Seu Certificado foi Emitido + Oferta Exclusiva (48h)"'::jsonb
  ),
  '{emailConfig,body}', 
...
-- Garantir que TODOS os templates tenham email ativado
UPDATE certificate_templates 
SET form_design = jsonb_set(
  COALESCE(form_design, '{}'::jsonb),
  '{emailConfig,enabled}', 
