...
-- Garantir que o sistema de envio de email esteja ativo para todos templates

BEGIN;

-- Primeiro, criar uma variável com o corpo do email HTML
DO $$
DECLARE
    email_body_html TEXT := '<p><strong>🎉 Parabéns {{nome_completo}}!</strong></p>
...
<p><strong>PPS:</strong> Lembre-se: como aluno, você tem prioridade e preços exclusivos que o público geral NUNCA terá acesso. Use este privilégio sabiamente.</p>';
BEGIN
    -- Atualizar TODOS os templates com as configurações de email padrão
      ...
      jsonb_set(
        jsonb_set(
          jsonb_set(
            ...
            to_jsonb('Marina L. - Therapist University'::text)
          ),
          '{emailConfig,senderEmail}', 
          to_jsonb('certificados@therapist.international'::text)
        ),
        '{emailConfig,subject}', 
        to_jsonb('🎉 Seu Certificado foi Emitido + Oferta Exclusiva (48h)'::text)
      ),
      '{emailConfig,body}', 
      to_jsonb(email_body_html)
    );

    RAISE NOTICE 'Todos os templates foram atualizados com o novo corpo de email HTML padrão';
END;
$$;

-- Garantir que TODOS os templates tenham email ativado (verificação adicional)
UPDATE certificate_templates 
SET form_design = jsonb_set(
  COALESCE(form_design, '{}'::jsonb),
  '{emailConfig,enabled}', 
...
-- Auditoria da database - verificar quantos templates foram atualizados
DO $$
DECLARE
    total_templates INTEGER;
    email_enabled_templates INTEGER;
    templates_with_new_body INTEGER;
BEGIN
    -- Contar total de templates
    SELECT COUNT(*) INTO total_templates FROM certificate_templates;
    
    -- Contar templates com email ativado
    SELECT COUNT(*) INTO email_enabled_templates 
    FROM certificate_templates 
    WHERE form_design->'emailConfig'->>'enabled' = 'true';
    
    -- Contar templates com o novo corpo de email
    SELECT COUNT(*) INTO templates_with_new_body 
    FROM certificate_templates 
    WHERE form_design->'emailConfig'->>'body' LIKE '%🎉 Parabéns {{nome_completo}}%';
    
    RAISE NOTICE '=== AUDITORIA DA DATABASE ===';
    RAISE NOTICE 'Total de templates: %', total_templates;
    RAISE NOTICE 'Templates com email ativado: %', email_enabled_templates;
    RAISE NOTICE 'Templates com novo corpo padrão: %', templates_with_new_body;
    RAISE NOTICE '=== SISTEMA DE EMAIL ATIVO PARA TODOS ===';
    
    -- Verificar se todos foram atualizados corretamente
    IF email_enabled_templates = total_templates AND templates_with_new_body = total_templates THEN
        RAISE NOTICE '✅ SUCESSO: Todos os templates foram atualizados corretamente!';
    ELSE
        RAISE NOTICE '⚠️  ATENÇÃO: Alguns templates podem não ter sido atualizados completamente';
    END IF;
END $$;
