...
-- 4. ÍNDICES OTIMIZADOS PARA PERFORMANCE (SE NÃO EXISTIREM)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_certificates_template_issued_at 
ON issued_certificates(template_id, issued_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_certificates_issued_at_month 
ON issued_certificates(DATE_TRUNC('month', issued_at));

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_templates_user_folder 
ON certificate_templates(user_id, folder_id, created_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_templates_user_active 
